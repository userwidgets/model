...
describe("User.Invite", () => {
	...
	const publicKey =
		...
		"mwIDAQAB"
	const creatable: model.User.Invite.Creatable = {
	...
	it("is", () => {
		const invite: model.User.Invite = {
			email: "jane@example.com",
			active: true,
			permissions: {
				"*": {
		...
		expect(model.User.Invite.is(invite)).toEqual(true)
	})
	it("signing", async () => {
		const issuer = model.User.Invite.Issuer.create(
			"userwidgets",
			"applicationId",
	...
	it("signing and verifying (authly)", async () => {
		const issuer = authly.Issuer.create<model.User.Invite.Creatable>(
			"userwidgets",
	...
	it("signing and verifying", async () => {
		const issuer = model.User.Invite.Issuer.create("userwidgets", "applicationId", publicKey, privateKey)
		const verifier = model.User.Invite.Verifier.create(publicKey)
		const token = await issuer.sign(creatable, Math.floor(now.getTime() / 1000))
		const result = await verifier.verify(token)
