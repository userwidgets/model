...
describe("Key", () => {
	...
	it("signing and verifying", async () => {
		...
		expect(await signer.verify("abc", await signer.sign("abc"))).toBe(true)
		expect(await signer.verify("cba", await signer.sign("abc"))).toBe(false)

		const issuer = model.User.Key.Issuer.create("userwidgets", "example", publicKey, privateKey)
		const verifierPublicKey = model.User.Key.Verifier.create(publicKey)
		const verifierNone = model.User.Key.Verifier.create()
