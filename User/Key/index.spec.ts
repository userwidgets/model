...
describe("Key", () => {
	...
	it("signing", async () => {
		const issuer = model.User.Key.Issuer.create(
			"userwidgets",
			"a",
	...
	it("signing and verifying", async () => {
		const issuer = model.User.Key.Issuer.create("userwidgets", "example", privateKey)
		const verifier = model.User.Key.Verifier.create("userwidgets")
		const token = await issuer.sign(creatable)
		const result = await verifier.verify(token)
	...
	it("unpack", async () => {
		const signedIssuer = model.User.Key.Issuer.create("userwidgets", "example", privateKey)
		const unsignedIssuer = model.User.Key.Issuer.create("local", "example")
